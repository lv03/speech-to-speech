...
* [STT module deep-dive](docs/stt.md)
* [LLM module deep-dive](docs/llm.md)
* [TTS module deep-dive](docs/tts.md)
...
| `voiceprint enroll` / `verify` / `info` | Enrolls or checks a speaker voiceprint for the security gate. | You want the assistant to unlock and respond only for your voice. |
...
Offline threshold calibration against your own target/non-target/overlap
recordings is strongly recommended before enabling the feature in production:
...
* `WS /v1/audio/transcriptions/stream` — streaming ASR: binary int16 PCM (16 kHz) frames in, `speech_started` / `partial` / `final` JSON events out. Select the backend via query params `model`, `language`, `hotwords`; send `{"type": "stop"}` to flush in-progress speech. `model=fun-asr-nano`/`paraformer` use VAD + re-transcription (clean growing text); `model=paraformer-online` uses true chunk streaming (`iic/speech_paraformer_asr_nat-zh-cn-16k-common-vocab8404-online`, ~600 ms chunks, incremental cache decoding) for low-latency partials plus an offline model (`final_model`, default `fun-asr-nano`) for the clean final result — the recommended dual-path approach.
