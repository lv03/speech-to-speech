"""Realtime 会话生命周期（Phase A 纯搬家，暂保持函数式接口）。

从 websocket_router 搬入：会话认领/释放/排空、取消与队列 flush 策略、
输出门控、usage salvage，以及每单元的输出泵（send loop）。

Phase A 仅做文件搬家，函数仍以 ``unit`` 为显式首参；后续 Phase 再收敛为
PipelineUnit 方法并私有化内部草稿字段。
"""
...
def claim_unit(pool: list[PipelineUnit], transport: SessionTransport | None) -> PipelineUnit | None:
...
async def send_loop_for(unit: PipelineUnit, stop_event: ThreadingEvent) -> None:
